...
class Wallet:
    ...
    def futures_worth(self , trading_coin_value):
        ...
        for order in self.futures_open_orders:
            if order.is_long:
                futures_worth += order.openning_amount * (  (trading_coin_value - order.openning_price)/ order.openning_price ) * order.leverage
            else:
                futures_worth -= order.openning_amount * (  (trading_coin_value - order.openning_price)/ order.openning_price ) * order.leverage
    ...
    def liguidation(self, trading_coin_value_min , trading_coin_value_max , Report):
        if self.futures_worth(trading_coin_value=trading_coin_value_min)<=0 :
            self.futures_open_orders.clear()
            Report.trace("LIQUIDATED")
        if self.futures_worth(trading_coin_value=trading_coin_value_max)<=0 :
            self.futures_open_orders.clear()
            Report.trace("LIQUIDATED")



    def buy(self, trading_pair , amount , price , Report ):
    ...
    def sell(self , trading_pair, amount , price , Report):
        execution_price = price * ( 1 + random.randrange(-1, 1) * random.random() * self.SLIPPAGE )
        if( self.COIN_DISPO > amount):
            self.usdt += amount * execution_price - self.FEES
            self.trading_coin -= amount
    ...
    def long(self, trading_pair , amount , price , leverage , Report):
        execution_price = price * ( 1 + random.randrange(-1, 1) *  random.random() * self.SLIPPAGE )
    ...
    def short(self, trading_pair , amount , price , leverage , Report):
        execution_price = price * ( 1 + random.randrange(-1, 1) *  random.random() * self.SLIPPAGE )
        if( self.FUTURES_FREE >= amount * execution_price + self.FEES):
    ...
    def close_all_open_order(self , trading_pair , price , Report):
        execution_price = price * ( 1 + random.randrange(-1, 1) *  random.random() * self.SLIPPAGE )
